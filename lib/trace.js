...
function checkPlaywrightVersion() {
  ...
  try {
    ...
    return null;                       // not installed; nothing to check against
  }
  const num = (v) => v.split('.').map(Number);
  const [a, b] = [num(version), num(VERIFIED_PLAYWRIGHT.min)];
  const [, c] = [null, num(VERIFIED_PLAYWRIGHT.max)];
  const lt = (x, y) => (x[0] - y[0]) || (x[1] - y[1]) || (x[2] - y[2]);
  if (lt(a, b) < 0 || lt(a, c) > 0) {
    console.warn(
      `[demo-recorder] playwright-core ${version} is outside the range this trace `
