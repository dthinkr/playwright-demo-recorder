 ...
 */
const fs = require('fs');
const { sanitizeStep } = require('./sanitize');

// The package's public CommonJS export is its browser-safe UMD bundle. Let
// Node resolve it so nested, hoisted and pnpm dependency layouts all work.
const SNAPSHOT_LIB = require.resolve('rrweb-snapshot');
...
class DemoRecorder {
   ...
   *
   * Dev-only chrome (Nuxt DevTools and Vite overlays) is stripped first — it
   * is an artefact of recording against a dev server and has no business in a
   * demo someone else opens.
   */
